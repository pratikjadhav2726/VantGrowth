...
import { incomingSignalSchema, routedSignalSchema, type IncomingSignal, type RoutedSignal } from "./contracts.js";
...
export const classifySignal = (signal: IncomingSignal): Omit<RoutedSignal, keyof IncomingSignal | "routedAt"> => {
...
export class SignalRouter {
  ...
  async route(input: IncomingSignal): Promise<RoutedSignal> {
    ...
    await this.deps.eventPublisher.publish(tenantScopedSubject(routed.tenantId, "signal.routed.v1"), payload);
