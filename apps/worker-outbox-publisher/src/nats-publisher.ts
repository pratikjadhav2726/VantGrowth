...
import { z } from "zod";
import type { EventPublisher } from "./outbox-publisher.js";

const natsConfigSchema = z.object({
  servers: z.string().min(1),
  name: z.string().default("growthos-worker-outbox-publisher"),
});
...
export const natsConfigFromEnv = (
  ...
  name: env.NATS_CLIENT_NAME ?? "growthos-worker-outbox-publisher",
});
