...
import { z } from "zod";
import type { EventPublisher } from "./attribution-worker.js";

const natsConfigSchema = z.object({
  servers: z.string().min(1),
  name: z.string().default("growthos-worker-attribution"),
});
...
export const natsConfigFromEnv = (
  ...
  name: env.NATS_CLIENT_NAME ?? "growthos-worker-attribution",
});
