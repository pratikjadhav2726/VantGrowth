...
export const paperclipEnvSchema = z.object({
  ...
  PAPERCLIP_TIMEOUT_MS: z
    .string()
    .regex(/^\d+$/)
    .optional()
});

export const paperclipConfigFromEnv = (env: Record<string, string | undefined>): PaperclipClientConfig => {
  ...
  return paperclipClientConfigSchema.parse({
    ...
    timeoutMs: parsed.PAPERCLIP_TIMEOUT_MS ? Number(parsed.PAPERCLIP_TIMEOUT_MS) : 15_000
  });
};
...
export type PaperclipCompanyCreateInput = z.infer<typeof paperclipCompanyCreateInputSchema>;
...
export type PaperclipAgentCreateInput = z.infer<typeof paperclipAgentCreateInputSchema>;
...
export type PaperclipIssueCreateInput = z.input<typeof paperclipIssueCreateInputSchema>;
...
export const paperclipCheckoutInputSchema = z.object({
  issueId: z.string().min(1),
  agentId: z.string().min(1),
  expectedStatuses: z.array(z.string().min(1)).default(["todo", "backlog", "blocked", "in_review"]),
  runId: z.string().optional()
});
export type PaperclipCheckoutInput = z.input<typeof paperclipCheckoutInputSchema>;
...
export class PaperclipClient implements PaperclipClientPort {
  ...
  async createCompany(input: PaperclipCompanyCreateInput): Promise<PaperclipCompany> {
  ...
  async createAgent(input: PaperclipAgentCreateInput): Promise<PaperclipAgent> {
    ...
    const result = await this.request("POST", `/api/companies/${body.companyId}/agents`, body);
  ...
  async createIssue(input: PaperclipIssueCreateInput): Promise<PaperclipIssue> {
    ...
    const result = await this.request("POST", `/api/companies/${body.companyId}/issues`, body);
  ...
  private async request(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Paperclip request failed ${response.status} ${response.statusText}: ${JSON.stringify(payload)}`);
      }
