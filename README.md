...
- `apps/api`: Hono API with async command acceptance pattern. Mounts OTel middleware (`createHttpMiddleware`) on every route; SDK initialised at startup via `initOtelSdk("growthos.api")`; structured pino logger replaces all `console.*` calls. Routes: **`POST /v1/motions/score`** (full motion scoring: validates `ScoringInput`, runs `scoreMotions()`, persists via `MotionStackRepository.recordScore()`, computes `stackUpdated` flag, 201 response); **`GET /v1/motion`** (motion overview: latest score + stack + history, `historyLimit` param, `X-Tenant-Id` header); `POST /v1/commands/outbox`, `POST /v1/workflows/*`, **`POST /v1/signals`** (signal ingest → `SignalEventsRepository`, idempotent), **`GET /v1/approvals`**, **`POST /v1/approvals/decide`**. 45 unit tests.
- `apps/web`: **`@growthos/web`** — Phase 1 / S5 UI. Next.js 15 App Router, Tailwind CSS 3, Inter font. Four pages: **Approval Queue** (`/approvals`) — server-fetched pending items, per-draft Approve/Reject via Server Actions + reviewer note; **Motion Stack** (`/motion`) — live data from `GET /v1/motion` with score grid, score delta vs prior run, history sparklines, stack config panel; **Signal Ingest** (`/signals`) — client-side form for 6 signal types with dedup feedback; **Weekly Review** (`/weekly-review`) — pipeline velocity stats, motion delta, pending queue, content pipeline list, founder weekly checklist. Typed `ApiClient` with Zod-validated responses (`listApprovals`, `submitApprovalDecision`, `ingestSignal`, `getMotionOverview`). Runs on port 3088.
