...
- `apps/api`: Hono API with async command acceptance pattern. Mounts OTel middleware (`createHttpMiddleware`) on every route; SDK initialised at startup via `initOtelSdk("growthos.api")`; structured pino logger replaces all `console.*` calls. Routes: **`POST /v1/motions/score`** (full motion scoring: validates `ScoringInput`, runs `scoreMotions()`, persists via `MotionStackRepository.recordScore()`, computes `stackUpdated` flag, 201 response); **`GET /v1/motion`** (motion overview: latest score + stack + history, `historyLimit` param, `X-Tenant-Id` header); **`POST /v1/signals/grade`** (LLM signal quality preview via `gradeSignalPayload`, requires `OPENAI_API_KEY` or injected `llmCallRunner`); **`GET /v1/digest/weekly`** + **`POST /v1/digest/send`** (weekly founder digest metrics + optional Postal send with safe dev fallback); `POST /v1/commands/outbox`, `POST /v1/workflows/*`, **`POST /v1/signals`** (signal ingest → `SignalEventsRepository`, idempotent), **`GET /v1/approvals`**, **`POST /v1/approvals/decide`**. 61 unit tests.
- `apps/web`: **`@growthos/web`** — Phase 1 / S5 UI. Next.js 15 App Router, Tailwind CSS 3, Inter font. Four pages: **Approval Queue** (`/approvals`) — server-fetched pending items, per-draft Approve/Reject via Server Actions + reviewer note; **Motion Stack** (`/motion`) — live data from `GET /v1/motion` with score grid, score delta vs prior run, history sparklines, stack config panel, **`MotionScoreForm`** (`postMotionScore` + refresh); **Signal Ingest** (`/signals`) — client-side form for 6 signal types with dedup feedback + optional LLM grade preview (`postSignalGrade`) before ingest; **session/auth layer** via signed cookie sessions (`src/lib/auth-token.ts`), `/login`, login/logout server actions, and route protection in `src/middleware.ts`; **Weekly Review** (`/weekly-review`) — pipeline velocity stats, motion delta, pending queue, content pipeline list, founder weekly checklist. Typed `ApiClient` with Zod-validated responses (`listApprovals`, `submitApprovalDecision`, `ingestSignal`, `getMotionOverview`, `postMotionScore`, `postSignalGrade`). Runs on port 3088.
...
- `packages/llm-harness`: **`@growthos/llm-harness`** — Phase 0 / Track D LLM abstraction layer. `PromptTemplate<TVars>` + `definePrompt()` factory; `LlmCallRunner` interface + `LlmCallRunOptions` (`tenantId`/`agentId`/`issueId` context fields); `StubLlmCallRunner` (deterministic test double); `OpenAiLlmCallRunner` (retry, OTel span, cost table, `fromEnv()`, **`fromSecrets(secretReader, tenantId)`** async factory that resolves API key from any duck-typed secrets reader with env fallback, optional `logSink`); **`signal-grade`** — `signalGradeSchema` + `gradeSignalPayload()` for `SIGNAL_GRADE_PROMPT` (shared by API `POST /v1/signals/grade` and signal-router worker); **7 versioned prompt templates**: `intel-brief.generate`, `content-brief.generate`, `blog-draft.generate`, `critique.evaluate`, `intel-brief.generate-structured` (JSON-only intel brief), `content-brief.generate-structured` (JSON-only content brief for LLM-backed ContentStrategist), `signal.grade` (relevance/urgency/category/recommendations grader); 48 unit tests. `LlmCallLogSink` pipeline (Noop + ClickHouse + Buffered); `LlmCallRunOptions` context forwarded to `buildLogRow()`.
- `packages/secrets`: **`@growthos/secrets`** — Phase 0 / Track E secrets infrastructure. `SecretManager` interface (`get/put/delete/list`); `EnvSecretManager` (dev/CI: `process.env` with POSIX→env-key mapping); `VaultSecretManager` (production: OpenBao/HashiCorp Vault KV v2 HTTP API, token auth via `VAULT_TOKEN`/`GROWTHOS_VAULT_TOKEN`, `fromEnv()` factory, 404→null, timeout); `TenantSecretsService` (tenant-scoped `tenants/{tenantId}/{key}` paths, `provisionTenant()` bulk-write, `listKeys()`); `secretPathSchema` input validation; 25 unit tests.
...
- `apps/worker-workflow-callback`: Workflow callback worker that emits tenant provisioning completion events. **Two execution paths**: (1) **Direct orchestrator path** — set `ENABLE_DIRECT_PROVISIONING=true` to run the `TenantProvisioningOrchestrator` 5-step sequence directly without Restate, with `OutboxProvisioningProgressReporter` writing progress to outbox + NATS; **step 6 (secrets)** — optional `tenantSecretsService` calls `provisionTenant()` post-provisioning, writing `openai_api_key`, `gitea_token`, `minio_*`, `nats_credentials` from env to secrets store; `resolveSecretsService()` selects `VaultSecretManager` vs `EnvSecretManager` via `VAULT_ADDR`/`VAULT_TOKEN`. (2) **Restate verification path** — queries Restate state and replays history. 9 tests. OTel SDK + pino logger wired.
...
- `apps/worker-critique`: **Critique worker** (upgraded Phase 1 S4) — **three-tier scoring**: (1) **LLM path** via `critiqueWithLlm()` — calls `CRITIQUE_EVALUATE_PROMPT`, parses `{verdict, confidence_score, reasons}` JSON (camelCase/snake_case normalised, defaults `confidenceScore=0.7`); optional `llmCallRunner` dep. (2) **Playbook rubric path** — `PlaybookVersionsRepository`-driven scoring via `evaluateCriterion()` (6 deterministic checks: has_cta, has_evidence, no_forbidden, length_ok, has_headings, has_hook) + `evaluateRubric()` (normalised weighted aggregation). (3) **Heuristic fallback** — length/reviewer-notes. 41 tests (10 LLM-path).
