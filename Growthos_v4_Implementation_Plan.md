...
1. **Track B groundwork**
   ...
   - **Next milestones:** Phase 1 / S5 (70% → 85%): `POST /v1/signals/grade` standalone endpoint; motion scoring form in web UI; Phase 0 / Track E (60% → 80%): AppRole/JWT auth upgrade for Vault; Phase 1 / S4 (95% → 100%): LLM rubric criterion evaluation per-criterion (swap deterministic check for LLM call).
