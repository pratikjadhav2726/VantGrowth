...
1. **Track B groundwork**
   ...
   - **Next milestones:** Phase 0 / Track E (30% → 60%): `TenantProvisioningOrchestrator` wired with `TenantSecretsService`; `SecretManager` injected into `OpenAiLlmCallRunner.fromEnv()` path; AppRole/JWT auth upgrade for Vault; Phase 1 / S5 remaining: session/auth layer, distribution integrations.
