...
1. **Track B groundwork**
   ...
   - **Next milestones:** Phase 0 / Track E (100%): materialize `growthos-runtime` + `ghcr-pull` via **External Secrets Operator** in staging/prod. Phase 1 / S5-S6: **persist tenant settings + digest prefs in Postgres** (replace dev cookies); multi-tenant CronJob or queue-driven digest dispatch.
