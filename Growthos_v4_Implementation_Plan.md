...
| Phase 1 / S2 (Motion Engine starter) | In progress (deterministic scorer v1 complete; **handoff contracts v0** wired; **skills library v0** 5 files; **`POST /v1/signals`** signal ingest (5 API tests); **LLM-backed `IntelDirectorWorker`**: `generateLlmBrief()` calls `INTEL_BRIEF_GENERATE_STRUCTURED_PROMPT`, JSON-parses + validates with `intelBriefV1Schema`, falls back to deterministic on parse failure; `StubLlmCallRunner` injected in tests; `callHistory` verified; `@growthos/llm-harness` dep added; 24 worker tests (+9 LLM-path tests)) | 80% |
...
1. **Track B groundwork**
   ...
   - **Next milestones:** Phase 1 / S2 remaining: `motion-scorer` upgrade to LLM-graded signals (SignalQualityGrader using `SIGNAL_GRADE_PROMPT`); Phase 1 / S5 remaining: session/auth layer, distribution integration; Phase 0 / Track E (Identity/billing/secrets/deploy).
