...
| Phase 0 / Track D (LLM + harness infra) | In progress (**`@growthos/llm-harness`** package: `PromptTemplate` type + `definePrompt()` factory (id + semver validation); `LlmCallRunner` interface + `LlmCallRunOptions` + `LlmCallResult`; **`StubLlmCallRunner`** deterministic test double with per-template configured responses + call history; **`OpenAiLlmCallRunner`** production runner with exponential-backoff retry (retryable on 429/5xx), OTel span per call (`llm.chat.completions`) with model/tokens/latency/cost attrs, static per-model cost estimation table + `registerModelPricing()` extension, prompt-cache detection via `usage.prompt_tokens_details`; 4 built-in versioned prompt templates (`intel-brief.generate`, `content-brief.generate`, `blog-draft.generate`, `critique.evaluate`); 24 unit tests: template (4), built-ins (3), stub runner (8), OpenAI runner (6), retry (2), pricing (1)) | 55% |
...
| Phase 1 / S2 (Motion Engine starter) | In progress (deterministic scorer v1 complete; **handoff contracts v0** wired: `intel_brief.v1`, `content_opportunity.v1`, `content_brief.v1`, `blog_draft.v1` schemas + registry + 22 tests; **skills library v0** 5 files) | 55% |
...
| Phase 1 / S5-S6 (UI, distribution) | Not started | 0% |
...
1. **Track B groundwork**
   ...
   - **Next milestones:** Phase 0 / Track D LLM harness Postgres `llm_call_logs` ClickHouse table + cost tracking sink; Phase 1 / S2 Motion Engine `motion-scorer` upgrade to LLM-graded signals; Phase 1 / S5 UI starter (approval queue + weekly review); Phase 0 / Track E (Identity/billing/secrets/deploy).
