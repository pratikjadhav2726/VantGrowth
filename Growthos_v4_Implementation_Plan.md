...
| Phase 0 / Track D (LLM + harness infra) | In progress (**`@growthos/llm-harness`** package: `PromptTemplate` + `definePrompt()` factory; `LlmCallRunner` interface; `StubLlmCallRunner`; `OpenAiLlmCallRunner` (retry, OTel, cost); 4 built-in prompt templates + **`INTEL_BRIEF_GENERATE_STRUCTURED_PROMPT`** + **`SIGNAL_GRADE_PROMPT`** (2 new production templates); **`signal-grade` module** — `signalGradeSchema`, `gradeSignalPayload()` shared by API + `SignalRouter`; 24 unit tests; **`LlmCallLogSink`** observability pipeline (Noop + ClickHouse + Buffered); `buildLogRow()` helper; **`LlmCallRunOptions.tenantId/agentId/issueId`** context fields forwarded to log sink; **`OpenAiLlmCallRunner.fromSecrets()`** async factory reads API key from any `{get(path): Promise<string\|null>}` resolver (Vault-backed in prod, env-fallback); 48 total unit tests) | 99% |
| Phase 0 / Track E (Identity/billing/secrets/deploy) | In progress (**`@growthos/secrets`** package: `SecretManager` interface (`get/put/delete/list`); `EnvSecretManager` (dev/CI: `process.env` with POSIX→env-key mapping); `VaultSecretManager` (production: OpenBao/Vault KV v2 HTTP API, token auth, timeout, `fromEnv()` factory, 404→null, non-2xx→throw); `TenantSecretsService` (tenant-scoped paths `tenants/{id}/{key}`, `provisionTenant()` bulk write, `listKeys()`); `secretPathSchema` validation; 25 unit tests; **`WorkflowCallbackWorker`** wired with optional `tenantSecretsService` — step 6 writes well-known secrets (openai_api_key, gitea_token, minio_*, nats_credentials) from env to secrets store post-provisioning; `resolveSecretsService()` in `index.ts` selects Vault vs Env based on `VAULT_ADDR`/`VAULT_TOKEN`; 2 new wiring tests (9 total worker-workflow-callback tests); **`VaultAppRoleAuth`** — non-human AppRole auth: `login()` POST `/v1/auth/approle/login`, `CachedToken` (token + expiresAt + leaseDuration + renewable), 30% renewal threshold, `renew()` POST `/v1/auth/token/renew-self`, re-login on renewal failure, `getTokenResolver()` returns `() => Promise<string>` for `VaultSecretManagerConfig.token`; `VaultSecretManager.token` upgraded to `string | (() => Promise<string>)` (backward-compatible); `fromEnv()` auto-detects AppRole (VAULT_ROLE_ID + VAULT_SECRET_ID) vs static token; 36 total secrets tests (11 new)) | 80% |
...
1. **Track B groundwork**
  ...
  - **Next milestones:** Phase 0 / Track E (80% → 100%): Zitadel identity + Lago billing + GitOps deploy (K3s/Argo CD); Phase 1 / S5-S6 (97% → 100%): onboarding wizard + founder digest delivery (Postal).
